...
import { TRACK_W, TRACK_LEN, BASE_SPEED, MAX_SPEED, PLAYER_Z, PLAYER_Y,
         BASE_FOV, TOUCH_SENS, roadX, rand } from './config.js';
...
addEventListener('keydown', e => {
  ...
  // 作弊码输入检测
  if (e.key && e.key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey && state === S.PLAY) {
    checkCheat(e.key);
  }
  if (e.code === 'Escape') {
    if (!ui.exitConfirm.classList.contains('hidden')) { exitResume(); return; }
    ...
    else if (state === S.PLAY) { togglePause(); ui.exitConfirm.classList.remove('hidden'); }
    else if (state === S.PAUSE) { ui.exitConfirm.classList.remove('hidden'); }
    else if (state === S.OVER) goHome();
  }
...
export function wireUI() {
  ...
  ui.returnQuizBtn.onclick = () => { returnToQuiz(Math.floor(score)); };
  ...
  ui.exitYesBtn.onclick = () => { sfx.click(); gameOver(); };
  ui.exitNoBtn.onclick = () => { sfx.click(); exitResume(); };
}
    ...
    if (!o.userData.hit && Math.abs(o.position.z - PLAYER_Z) < 2.2) {
      ...
      if (dx * dx + dy * dy < o.userData.r * o.userData.r) {
        ...
        if (o.userData.isRing) {
          ...
          if (shieldT > 0 || window.__invincible) {
            if (!window.__invincible) {
