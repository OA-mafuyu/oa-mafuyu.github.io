...
// ---------- 从答题中启动检测 ----------
var fromQuiz = !!localStorage.getItem('quiz_minigame_history');

// 更新返回按钮文字（起始页 / 退出弹窗）
(function updateReturnButtonTexts() {
  var splashHome = document.getElementById('btn-splash-home');
  if (splashHome) {
    splashHome.textContent = fromQuiz ? '返回答题页面' : '返回主页';
    splashHome.onclick = function () {
      if (fromQuiz) {
  ...
  if (exitConfirmBtn) {
    exitConfirmBtn.textContent = fromQuiz ? '返回答题页面' : '退出';
  }
})();

// ---------- 击杀里程碑（每个仅触发一次） ----------
var killMilestonesHit = {};

function checkKillMilestones() {
  var bonus = 0;
  if (killCount >= 50 && !killMilestonesHit[50]) { bonus = 15; killMilestonesHit[50] = true; }
  else if (killCount >= 20 && !killMilestonesHit[20]) { bonus = 10; killMilestonesHit[20] = true; }
  else if (killCount >= 5  && !killMilestonesHit[5])  { bonus = 5;  killMilestonesHit[5]  = true; }
  if (bonus > 0) {
    var el = document.getElementById('hud-kills');
    if (el) { el.classList.add('kill-pop'); setTimeout(function () { el.classList.remove('kill-pop'); }, 500); }
  }
  return bonus;
}

function updateKillHUD() {
  var c = window.killCount;
  var hudCnt = document.getElementById('hud-kill-count');
  if (hudCnt) hudCnt.textContent = String(c);
  checkKillMilestones();
}

function calcKillBonusTime() {
  var bonus = 0;
  if (killCount >= 50) bonus = 15;
  else if (killCount >= 20) bonus = 10;
  else if (killCount >= 5)  bonus = 5;
  return bonus;
}

function returnToQuiz() {
  var bonus = calcKillBonusTime();
  try {
...
function showReturnQuizBtnFn() {
  var btn = document.getElementById('return-quiz-btn');
  if (btn) {
    if (fromQuiz) {
      var bonus = calcKillBonusTime();
      btn.textContent = '返回答题 (+' + bonus + '秒)';
    } else {
...
function checkCheat(key) {
  ...
  // 限制 buffer 长度
  if (cheatBuffer.length > 40) cheatBuffer = cheatBuffer.slice(-30);
}

window.addEventListener('keydown', e => {
  const k = e.key.toLowerCase();
  // 作弊码：仅在游戏运行且未结束时生效
  if (gameStarted && !gameOver && e.key && e.key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey) {
    checkCheat(e.key);
  }
  // ESC 退出确认（仅在游戏运行且未结束时）
  if (k === 'escape' && gameStarted && !gameOver) {
    e.preventDefault();
    showExitOverlay();
    return;
  }
...
function startGame() {
  ...
  // 显示击杀 HUD
  document.getElementById('hud-kills').classList.add('visible');
  updateKillHUD();
...
function gameLoop() {
  // 作弊码超时重置
  if (cheatTimer > 0) { cheatTimer -= 0.016; if (cheatTimer <= 0) cheatBuffer = ''; }
...
function hideEndButtons() {
  ...
  document.getElementById('return-quiz-btn').classList.remove('visible');
}
...
// 返回答题 / 返回主页按钮
document.getElementById('return-quiz-btn').addEventListener('click', () => {
  if (fromQuiz) {
    returnToQuiz();
  } else {
    window.location.href = '../index.html';
...
// ---------- 退出确认遮罩 ----------
var _exitOverlayPrevPaused = false;

function showExitOverlay() {
  // 暂停游戏
  _exitOverlayPrevPaused = paused;
  if (!paused) {
    paused = true;
    updatePauseBtnUI();
...
document.getElementById('exit-cancel-btn').addEventListener('click', hideExitOverlay);
document.getElementById('exit-confirm-btn').addEventListener('click', confirmExitGame);
